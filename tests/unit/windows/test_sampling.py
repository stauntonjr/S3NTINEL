...
from libs.windows import WindowCoverageSampler
...
def test_sample_windows_for_coverage_respects_target_size_per_flight():
    ...
    out = WindowCoverageSampler(sample_size_per_flight=6, bins_per_axis=3).sample(windows)
    assert len(out) == 6
...
def test_sample_windows_for_coverage_samples_each_flight_independently():
    ...
    out = WindowCoverageSampler(sample_size_per_flight=4, bins_per_axis=2).sample(windows)
    by_flight = {}
    for item in out:
