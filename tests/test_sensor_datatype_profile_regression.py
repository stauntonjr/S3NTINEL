...
def test_build_sensor_datatype_profile_infers_numeric_binary_and_categorical(spark):
    rows = [
        ("S_NUM", "1.1"),
        ("S_NUM", "2.5"),
        ("S_NUM", "3.2"),
    ...
    prof_df = build_sensor_datatype_profile(telemetry_df)
    results = {row["sensor"]: row["detected_type"] for row in prof_df.select("sensor", "detected_type").collect()}
...
def test_build_sensor_datatype_profile_supports_parameter_name_column(spark):
    rows = [
        ("P1", "10.0"),
        ("P1", "11.0"),
        ("P2", "ON"),
