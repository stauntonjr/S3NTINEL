...
from libs.windows import build_window_features_spark_table, build_window_features_with_diagnostics_spark_table
import pandas as pd
...
def _build_window_features_pdf(raw_df, events_df, windows_df) -> pd.DataFrame:
    ...
    return (
        build_window_features_spark_table(raw_df, events_df, windows_df)
        .toPandas()
...
def _build_phase_artifacts_pdf(raw_df, events_df, windows_df):
    ...
    window_features_sdf = build_window_features_spark_table(raw_df, events_df, windows_df)
    ...
    phase_windows_pdf = (
        build_phase_windows_spark_table(window_features_sdf, phase_config=config, phase_count=2)
        .toPandas()
        .sort_values(["tail_id", "flight_id", "win_id"], kind="stable")
        .reset_index(drop=True)
    )
...
def test_build_window_features_with_diagnostics_spark_table_reports_step_details(spark):
    ...
    window_features_sdf, diagnostics = build_window_features_with_diagnostics_spark_table(raw_df, events_df, windows_df)
...
def test_fit_phase_feature_config_from_spark_builds_backbone_weights(spark):
    ...
    events_df = create_sample_events_df(spark)
    windows_df = create_sample_windows_df(spark)
    window_features_df = build_window_features_spark_table(raw_df, events_df, windows_df)
...
def test_build_phase_spark_tables_match_python_phase_runtime(spark):
    ...
    phase_baselines_sdf = build_phase_baselines_spark_table(phase_windows_sdf, phase_config=config)
...
def test_fit_cluster_model_falls_back_when_no_windows_are_stable(spark):
    ...
    events_df = create_sample_events_df(spark)
    windows_df = create_sample_windows_df(spark)
    window_features_sdf = build_window_features_spark_table(raw_df, events_df, windows_df).withColumn(
...
def test_phase_detection_plan_build_keeps_phase_config_as_domain_object(spark):
    ...
    assert artifacts.phase_windows_df.count() == window_features_sdf.count()
    assert artifacts.phase_baselines_df.count() >= 1
...
def test_build_window_scores_raw_spark_table_uses_phase_artifacts(spark):
    ...
    expected_scores_df = (
        build_window_scores_raw_table(
            phase_windows_pdf,
            phases_pdf,
            hierarchy_sensor_map_df.toPandas(),
        )
        .sort_values(["tail_id", "flight_id", "win_id"], kind="stable")
        .reset_index(drop=True)
    )
