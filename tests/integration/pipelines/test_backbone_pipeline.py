...
from libs.windows import build_window_features_dataframe, window_features_pandas_to_spark_dataframe
from libs.testing.data import create_sample_raw_table_df, create_sample_windows_df
...
def test_build_backbone_sensor_energy_spark_table_matches_pandas_builder(spark):
    ...
    empty_events_df = raw_df.iloc[0:0].copy()
    empty_events_df = empty_events_df.assign(event_type_detected="", payload=None)
    window_x_pdf = build_window_features_dataframe(raw_df, empty_events_df, windows_df)
    window_x_sdf = window_features_pandas_to_spark_dataframe(spark, window_x_pdf)
