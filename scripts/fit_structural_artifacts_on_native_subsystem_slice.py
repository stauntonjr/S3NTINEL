"""Fit backbone and graph artifacts on a native subsystem slice."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fit backbone and graph artifacts on a native subsystem slice")
...
def main() -> None:
    ...
    spark = get_spark("s3ntinel.fit_structural_artifacts_on_native_subsystem_slice")

    backbone_artifacts, phase_df = simulate_native_backbone_artifacts_from_subsystem_slice(
    ...
    graph_artifacts, _ = simulate_native_graph_artifacts_from_subsystem_slice(
        slice_name=str(args.slice_name),
        spark=spark,
        tail_id=str(args.tail_id),
        flight_id=str(args.flight_id),
    ...
    for artifact_name, artifact_df in backbone_artifacts.items():
        print(f"{artifact_name}_rows={len(artifact_df)}")
    for artifact_name, artifact_df in graph_artifacts.items():
        print(f"{artifact_name}_rows={len(artifact_df)}")
    print("backbone_preview")
    print(backbone_artifacts["backbone"].to_string(index=False))
    print("hierarchy_preview")
    print(graph_artifacts["hierarchy_sensor_map"].to_string(index=False))
