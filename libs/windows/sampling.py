"""Window sampling helpers for broad state-space coverage."""
...
def sample_windows_for_coverage(
    windows: Iterable[dict[str, Any]],
    *,
    sample_size_per_flight: int = 32,
    bins_per_axis: int = 4,
) -> list[dict[str, Any]]:
    """Sample windows with broad 3D coverage over drift, duration, and clock time."""
    sample_target = max(int(sample_size_per_flight), 0)
    if sample_target <= 0:
        return []
    ...
    for flight_key in sorted(by_flight.keys(), key=lambda item: (item[0], item[1])):
        ...
        for item in enriched:
            ...
            item["coords"] = (x, y, z)
            bucket = (_bin_index(x, bins_per_axis), _bin_index(y, bins_per_axis), _bin_index(z, bins_per_axis))
        ...
        # Pass 1: roughly uniform coverage by bucket.
        exhausted = False
        ...
        # Pass 2: farthest-point fill to span the convex hull.
        ...
        while remaining and len(local_selected) < target:
            ...
            for idx, item in enumerate(remaining):
                ...
                if min_dist_sq > best_score or (abs(min_dist_sq - best_score) <= 1e-12 and (best_tie is None or tie < best_tie)):
