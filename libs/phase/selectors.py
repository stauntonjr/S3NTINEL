...
def select_event_types_from_window_features_spark(window_features_df: "DataFrame", *, k: int) -> list[str]:
    ...
    counts_df = event_counts_from_window_features_spark(window_features_df)
    ...
    continuous_rows = (
        ...
        .orderBy(F.col("event_count").desc(), F.col("event_type").asc())
        .limit(continuous_k)
        .collect()
    )
...
def select_categorical_state_pairs_from_window_features_spark(
    ...
    *,
    ...
    rows = (
        ...
        .orderBy(F.col("pair_count").desc(), F.col("parameter_name").asc(), F.col("state").asc())
        .limit(limit)
        .collect()
    )
    return [(str(row["parameter_name"]), str(row["state"])) for row in rows]
...
def select_categorical_state_pairs_with_diagnostics_from_window_features_spark(
    ...
    *,
    ...
    rows = (
        pair_counts_df.orderBy(F.col("pair_count").desc(), F.col("parameter_name").asc(), F.col("state").asc())
        .limit(limit)
        .collect()
    )
