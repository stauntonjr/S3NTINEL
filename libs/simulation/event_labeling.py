...
def _detect_sensor_event_labels_df(telemetry_df: pd.DataFrame, *, value_col: str | None = None) -> pd.DataFrame:
    """Build sensor-level detector event labels from telemetry samples.

    This emits per-(tail, flight, sensor, timestamp) event labels aligned to the
    ...
    if telemetry_df.empty:
        return pd.DataFrame(columns=["tail_id", "flight_id", "sensor", "timestamp_utc", "event_type_label"])

    out_df = telemetry_df.copy()
    if value_col and str(value_col) in out_df.columns and str(value_col) != "parameter_value_clean":
        ...
        event_rows.append(
            {
                ...
                "timestamp_utc": pd.to_datetime(event.get("ts"), utc=True),
...
def _attach_event_labels_to_telemetry_df(telemetry_df: pd.DataFrame, *, label_value_col: str = "parameter_value_clean") -> pd.DataFrame:
    ...
    labels_df = _detect_sensor_event_labels_df(out_df, value_col=active_label_value_col)
    if labels_df.empty:
        out_df["event_type_label"] = EventType.NONE
        return out_df

    grouped = (
        labels_df.groupby(["tail_id", "flight_id", "sensor", "timestamp_utc"])["event_type_label"]
    ...
    out_df = out_df.merge(
        grouped[["tail_id", "flight_id", "sensor", "timestamp_utc", "event_type_label"]],
        on=["tail_id", "flight_id", "sensor", "timestamp_utc"],
        how="left",
    )
...
def _attach_event_labels_to_telemetry_rows(telemetry_rows: list[dict], *, label_value_col: str = "parameter_value_clean") -> list[dict]:
    ...
    if active_label_value_col != "parameter_value_clean":
        for row in out_rows:
            row["parameter_value_clean"] = row.get(active_label_value_col)
