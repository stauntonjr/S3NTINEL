...
def build_parameter_profile(
    ...
    prof = (
        ...
        .withColumn(
            "detected_type",
            F.when(F.col("distinct_value_count") <= 1, F.lit("constant"))
            .when((F.col("numeric_rate") >= F.lit(numeric_ratio_threshold)) & (F.col("distinct_value_count") > 2), F.lit("numeric"))
            .when(F.col("distinct_value_count") == 2, F.lit("binary"))
            .when(F.col("distinct_value_count") <= F.lit(int(categorical_cardinality_max)), F.lit("categorical"))
            .otherwise(F.lit("high_cardinality")),
        )
        .select(
            "parameter_name",
            "detected_type",
