...
def run() -> None:
    ...
    max_reference_rows = settings.scoring.max_bridge_reference_rows
    ...
    if phase_baselines_count > max_reference_rows or hierarchy_sensor_map_count > max_reference_rows:
        raise RuntimeError(
            "80_window_scores_raw only allows bounded reference-table collection; "
    ...
    log_dict_artifact_if_active(
        ...
        "reports/stages/80_window_scores_raw_summary.json",
    )
    ...
    log_stage_manifest_if_active(stage_manifest, "reports/stages/80_window_scores_raw_manifest.json")
    LOGGER.info(
