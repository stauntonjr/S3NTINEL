...
def _build_cur_matrices_report(
    *,
    cur_column_sketch_path: str,
    cur_column_leverage_path: str,
    cur_row_sketch_path: str,
    cur_sensor_sample_path: str,
    cur_row_sample_path: str,
    cur_c_matrix_path: str,
    cur_r_matrix_path: str,
    cur_w_matrix_path: str,
    cur_u_matrix_path: str,
    column_sketch_count: int,
    column_leverage_count: int,
    row_sketch_count: int,
    cur_sampling_mode: str,
    cur_sampling_seed: int,
    ...
    u_nnz: int,
    u_core_meta: dict,
    effective_pivots_k: int,
    effective_row_samples_k: int,
    cur_u_contraction_mode: str,
) -> dict:
    return {
        ...
        "c_nnz": c_nnz,
        "u_contraction_mode": cur_u_contraction_mode,
        "r_nnz": r_nnz,
        "w_nnz": w_nnz,
        "u_nnz": u_nnz,
        "u_core": u_core_meta,
...
def run() -> None:
    ...
    precision_graph_path = os.getenv("S3NTINEL_PRECISION_GRAPH_TABLE_PATH", "data/delta/precision_sensor_graph")
    ...
    hierarchy_nodes_path = os.getenv("S3NTINEL_HIERARCHY_NODES_TABLE_PATH", "data/delta/hierarchy_nodes")
    hierarchy_edges_path = os.getenv("S3NTINEL_HIERARCHY_EDGES_TABLE_PATH", "data/delta/hierarchy_edges")
    hierarchy_sensor_map_path = os.getenv("S3NTINEL_HIERARCHY_SENSOR_MAP_TABLE_PATH", "data/delta/sensor_hierarchy_map")
    hierarchy_profile_id = os.getenv("S3NTINEL_HIERARCHY_PROFILE_ID", "HIER_FIT_V1")
    hierarchy_source = os.getenv("S3NTINEL_HIERARCHY_SOURCE", "spark_pic_multi_level")
    ...
    cur_u_contraction_mode = str(os.getenv("S3NTINEL_CUR_U_CONTRACTION_MODE", "pivot_restricted_a")).strip().lower()
    ...
    min_abs_partial_corr = float(
        os.getenv(
            "S3NTINEL_PRECISION_GRAPH_MIN_ABS_PARTIAL_CORR",
            str(context.config["graph"].get("min_abs_partial_corr", context.config["graph"]["min_abs_corr"])),
        )
    )
    precision_ridge_lambda = float(
        os.getenv(
            "S3NTINEL_PRECISION_GRAPH_RIDGE_LAMBDA",
            str(context.config["graph"].get("precision_ridge_lambda", 1e-6)),
        )
    )
    subsystem_min_edge_weight = float(
        os.getenv(
            "S3NTINEL_SUBSYSTEM_MIN_EDGE_WEIGHT",
            str(context.config["graph"].get("subsystem_min_edge_weight", 0.2)),
        )
    )
    hierarchy_k_system = int(os.getenv("S3NTINEL_HIERARCHY_K_SYSTEM", str(context.config["graph"].get("hierarchy_k_system", 8))))
    hierarchy_k_subsystem = int(
        os.getenv("S3NTINEL_HIERARCHY_K_SUBSYSTEM", str(context.config["graph"].get("hierarchy_k_subsystem", 24)))
    )
    hierarchy_k_module = int(os.getenv("S3NTINEL_HIERARCHY_K_MODULE", str(context.config["graph"].get("hierarchy_k_module", 64))))
    ...
    u_matrix_df, u_core_meta = build_u_core_from_w(
        ...
        a_matrix_df=a_matrix_df,
        c_matrix_df=c_matrix_df,
        r_matrix_df=r_matrix_df,
        ...
        contraction_mode=cur_u_contraction_mode,
    )
    ...
    precision_graph_df = build_precision_sensor_graph(
        raw_df,
        normalization_df,
        sampled_sensors_df=sampled_sensors_df.select("sensor"),
        max_sensors=max_sensors,
        min_overlap=min_overlap,
    ...
    hierarchy_sensor_map_df, hierarchy_nodes_df, hierarchy_edges_df, hierarchy_meta = build_sensor_hierarchy_from_graphs(
        spark,
        sampled_sensors_df.select("sensor"),
        fused_graph_df,
        precision_graph_df,
        min_edge_weight=subsystem_min_edge_weight,
        k_system=hierarchy_k_system,
        k_subsystem=hierarchy_k_subsystem,
        k_module=hierarchy_k_module,
        hierarchy_profile_id=hierarchy_profile_id,
        hierarchy_source=hierarchy_source,
    )
    subsystem_map_df = hierarchy_sensor_map_df.select("sensor", "subsystem_id")
    ...
    write_table(precision_graph_df, path=precision_graph_path, mode=write_mode, fmt=table_format)
    ...
    write_table(subsystem_map_df, path=subsystem_map_path, mode=write_mode, fmt=table_format)
    write_table(hierarchy_sensor_map_df, path=hierarchy_sensor_map_path, mode=write_mode, fmt=table_format)
    write_table(hierarchy_nodes_df, path=hierarchy_nodes_path, mode=write_mode, fmt=table_format)
    write_table(hierarchy_edges_df, path=hierarchy_edges_path, mode=write_mode, fmt=table_format)
    ...
    precision_edges = precision_graph_df.count()
    event_edges = event_graph_df.count()
    fused_edges = fused_graph_df.count()
    subsystem_count = subsystem_map_df.select("subsystem_id").distinct().count()
    ...
    report_payload = {
        ...
        "precision_graph": {
            "path": precision_graph_path,
            "min_abs_partial_corr": min_abs_partial_corr,
            "ridge_lambda": precision_ridge_lambda,
            "edge_count": precision_edges,
        },
        "cur_matrices": _build_cur_matrices_report(
            cur_column_sketch_path=cur_column_sketch_path,
            cur_column_leverage_path=cur_column_leverage_path,
            cur_row_sketch_path=cur_row_sketch_path,
            cur_sensor_sample_path=cur_sensor_sample_path,
            cur_row_sample_path=cur_row_sample_path,
            cur_c_matrix_path=cur_c_matrix_path,
            cur_r_matrix_path=cur_r_matrix_path,
            cur_w_matrix_path=cur_w_matrix_path,
            cur_u_matrix_path=cur_u_matrix_path,
            column_sketch_count=column_sketch_count,
            column_leverage_count=column_leverage_count,
            row_sketch_count=row_sketch_count,
            ...
            w_nnz=w_nnz,
            u_nnz=u_nnz,
            u_core_meta=u_core_meta,
            effective_pivots_k=effective_pivots_k,
            effective_row_samples_k=effective_row_samples_k,
            cur_u_contraction_mode=cur_u_contraction_mode,
        ),
        "event_graph": {
        ...
        "subsystem_map": {
            "path": subsystem_map_path,
            "min_edge_weight": subsystem_min_edge_weight,
            "subsystem_count": subsystem_count,
        },
        "hierarchy": {
            "sensor_map_path": hierarchy_sensor_map_path,
            "nodes_path": hierarchy_nodes_path,
            "edges_path": hierarchy_edges_path,
            "profile_id": hierarchy_profile_id,
    ...
    log_params_if_active(
        {
            ...
            "precision_graph_edges": precision_edges,
            "event_graph_edges": event_edges,
            "fused_graph_edges": fused_edges,
            "subsystem_map_count": subsystem_count,
            "subsystem_min_edge_weight": subsystem_min_edge_weight,
            "hierarchy_sensor_count": int(hierarchy_meta.get("sensor_count", 0)),
            "hierarchy_system_count": int(hierarchy_meta.get("system_count", 0)),
            "hierarchy_subsystem_count": int(hierarchy_meta.get("subsystem_count", 0)),
            "hierarchy_module_count": int(hierarchy_meta.get("module_count", 0)),
        }
    )
    LOGGER.info(
        "pipeline=cur_backbone_fit input=%s norm_sensors=%s sampled_sensors=%s sampled_rows=%s cur_edges=%s precision_edges=%s event_edges=%s fused_edges=%s subsystems=%s systems=%s modules=%s norm=%s c=%s r=%s u=%s cur_graph=%s precision_graph=%s event_graph=%s fused_graph=%s subsystem_map=%s hierarchy_map=%s hierarchy_nodes=%s hierarchy_edges=%s report=%s",
        input_path,
        ...
        fused_edges,
        subsystem_count,
        int(hierarchy_meta.get("system_count", 0)),
        int(hierarchy_meta.get("module_count", 0)),
        normalization_path,
        cur_c_matrix_path,
        cur_r_matrix_path,
        cur_u_matrix_path,
        cur_graph_path,
        precision_graph_path,
        event_graph_path,
        fused_graph_path,
        subsystem_map_path,
        hierarchy_sensor_map_path,
        hierarchy_nodes_path,
